...
print("Configuración servicio mosquitto")
sudo systemctl start mosquitto.service && \
sudo systemctl enable mosquitto.service && \
echo "mqtt_server_uri = localhost:1883" | sudo tee -a /etc/openvas/openvas.conf

print("Creacion de los directorios faltantes")
sudo -v
sudo mkdir -p /var/lib/notus && \
sudo mkdir -p /run/notus-scanner && \
sudo mkdir -p /run/gvmd

print("Añadir gvm al grupo de redis y permisos")
sudo -v
...
print("ajustar permisos del feed")
sudo -v
...
print("Permitir a los usuarios del grupo GVM poder lanzar openvas")
sudo -v
# allow users of the gvm group run openvas
sudo echo -e "gvm ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers


print("Arrancamos postgresql")
sudo -v
...
print("Recargamos loader cache dinamica")
sudo -v
sudo ldconfig

print("Creamos usuario admin/admin")
...
print("Generamos los certificados https")
sudo -v
sudo -u gvm gvm-manage-certs -a

print("Creamos servicio gvmd")
sudo -v
...
print("Creamos servicio ospd-openvas")
sudo -v
...
print("Comprobamos status de los servicios")
